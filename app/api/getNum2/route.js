    ...
    try {
        ...
        if (!data) {
            ...
            if (data.time2number === null || data.time2number === 0) {
                ...
                const random4DigitNumber = Math.floor(1000 + Math.random() * 9000);
                const paddedNumber = random4DigitNumber.toString().padStart(4, '0');
                data.time2number = paddedNumber;
                await data.save();
                return new Response(JSON.stringify({ timeNumber: paddedNumber }), {
                    status: 200,
                });
            } else {
                // If time2number is not null or 0, return the existing value as a 4-digit string
                const paddedNumber = data.time2number.toString().padStart(4, '0');
                return new Response(JSON.stringify({ timeNumber: paddedNumber }), {
                    status: 200,
                });
            }
